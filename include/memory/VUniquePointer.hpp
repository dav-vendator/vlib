...
//Specialisation for array types
    ...
    template<typename T>
    VUniquePointer<T[]>::VUniquePointer(VUniquePointer<T[]>&& other) noexcept : ptr_(other.ptr_){
        other.ptr_ = nullptr;
    }
    
    template<typename T>
    VUniquePointer<T[]>& VUniquePointer<T[]>::operator=(VUniquePointer<T[]>&& other) noexcept{
        if (this == &other)
            return *this;
    ...
    template<typename T>
    T& VUniquePointer<T[]>::operator[](std::size_t index) const{
